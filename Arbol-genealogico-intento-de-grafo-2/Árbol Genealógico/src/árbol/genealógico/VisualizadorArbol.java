...
public class VisualizadorArbol {
    ...
    public void limpiarResaltado() {
        // Restaurar el estilo del nodo resaltado si existe
        if (parent != null) {
            Node nodoAnterior = graph.getNode(parent);
            if (nodoAnterior != null) {
                // Primero eliminar cualquier estilo personalizado
                nodoAnterior.removeAttribute("ui.style");
                
                // Restaurar el estilo según la casa
                TablaHash.Persona[] personas = tablaPersonas.obtenerTodasLasPersonas();
                for (TablaHash.Persona persona : personas) {
                    if (persona != null && 
                        obtenerPrimerNombre(persona.nombreCompleto).equals(parent)) {
                        if (persona.casa != null) {
                            nodoAnterior.setAttribute("ui.class", persona.casa.toLowerCase());
                        } else {
                            // Si no tiene casa, establecer el estilo por defecto
                            nodoAnterior.setAttribute("ui.class", null);
                            nodoAnterior.setAttribute("ui.style", 
                                "fill-color: white; " +
                                "size: 30px; " +
                                "stroke-mode: plain; " +
                                "stroke-color: black;");
